...
- 可选 `CLAUDE.md`、Codex instructions 和 Cursor rule：给 Claude Code / Codex / Cursor 的轻量 handoff。
- `scan --format json`：给脚本和下游工具复用的机器可读仓库地图。

默认输出不绑定任何模型、供应商或 agent 工具；你可以先用纯 Markdown 工作，再按需生成 Claude Code、Codex 或 Cursor 适配文件。
...
生成 Claude Code、Codex 和 Cursor handoff：

```powershell
agent-workbench init . --output .agent-workbench --adapter claude --adapter codex --adapter cursor
```
...
- 最新 release: <https://github.com/Xiao-rx/agent-workbench/releases/tag/v0.7.0>
