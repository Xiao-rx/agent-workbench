...
跑完 `agent-workbench demo --report` 或 `agent-workbench init --report` 后，可以复制报告里的分享摘要，并使用 **Share Feedback** 链接打开 Agent Workbench report issue。请不要包含 token、`.env.local`、`.env.bak`、私有仓库名或专有源码片段。
