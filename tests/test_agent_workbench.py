...
class AgentWorkbenchTests(unittest.TestCase):
    ...
    def test_demo_command_report_shortcut_writes_shareable_markdown(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("- Workbench: `<workbench>`", text)
            self.assertIn("- Report command: `agent-workbench demo --output <demo-workspace> --report <report>`", text)
            ...
            self.assertIn("`agent-workbench check <repo> --workbench <workbench>", text)
            ...
            self.assertNotIn(str(output.resolve()), text)
            self.assertNotIn(str(report), text)
    ...
    def test_init_command_report_shortcut_writes_shareable_markdown(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("- Repository: `<repo>`", text)
            self.assertIn("- Workbench: `<workbench>`", text)
            self.assertIn("- Report command: `agent-workbench init <repo> --output <workbench> --report <report>`", text)
            self.assertIn("`AGENTS.md`", text)
            self.assertIn("`.codex/AGENTS.md`", text)
