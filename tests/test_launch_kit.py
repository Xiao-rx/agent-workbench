...
class LaunchKitTests(unittest.TestCase):
    def test_launch_kit_contains_shareable_assets(self):
        ...
        self.assertIn("agent-workbench demo --adapter all --check --format json", launch)
        self.assertIn("agent-workbench init . --output .agent-workbench --adapter all --check --format json", launch)
