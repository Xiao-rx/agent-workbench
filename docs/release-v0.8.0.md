...
- Markdown reports now redact local filesystem paths to `<repo>`, `<workbench>`, `<report>`, and `<demo-workspace>` placeholders, while JSON proofs keep exact paths for CI and downstream agent harnesses.
- `demo --template typescript --report`: run the same no-secret proof/report path on a tiny Node/npm TypeScript repository, with `npm test` as the verification command.
